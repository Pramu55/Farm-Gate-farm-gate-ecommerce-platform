...
import type { MysqlPool } from './db';

const SALT_ROUNDS = 12;

export function createAuthRouter(pool: MysqlPool) {
  const router = Router();

  router.post('/register', async (req: Request, res: Response) => {
    try {
      const { role, name, phone, address, state, district, company, license, pan, bank, password } = req.body as any;
      ...
      if (role === 'farmer') {
        const [result] = await pool.execute(
          ...
          [name ?? '', phone, address ?? '', state ?? '', district ?? '', pan ?? '', bank ?? 0, passwordHash]
        );
      ...
      if (role === 'buyer') {
        const [result] = await pool.execute(
          'INSERT INTO buyerregistration (buyer_name, buyer_phone, buyer_addr, buyer_comp, buyer_license, buyer_bank, buyer_pan, buyer_mail, buyer_username, buyer_password) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
          [name ?? '', phone, address ?? '', company ?? '', license ?? '', bank ?? 0, pan ?? '', '', String(phone), passwordHash]
        );
  ...
  router.post('/login', async (req: Request, res: Response) => {
    try {
      const { role, phone, password } = req.body as any;
      if (!role || !phone || !password) {
        return res.status(400).json({ error: 'role, phone, password required' });
      }
      ...
      const token = jwt.sign({ sub: String(row.id), role }, process.env.JWT_SECRET as string, { expiresIn: '7d' });
