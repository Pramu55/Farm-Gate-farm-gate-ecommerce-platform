...
const locations = new Map<string, { role: 'dealer' | 'consumer' | 'farmer'; lat: number; lon: number; label?: string }>();
...
app.post('/location', (req: Request, res: Response) => {
  const { id, role, lat, lon, label } = req.body as { id: string; role: 'dealer' | 'consumer' | 'farmer'; lat: number; lon: number; label?: string };
